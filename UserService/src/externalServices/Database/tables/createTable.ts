...
class CreateTable {
    ...
    try {
      ...
      if (!tableExists) {
        await this.dbConnector.schema
          .withSchema(environmentVariables.SCHEMA_NAME)
  ...
  private async checkIfTableExists(tableName: string): Promise<boolean> {
    try {
      const tableExists = await this.dbConnector.schema.withSchema(environmentVariables.SCHEMA_NAME).hasTable(tableName);
      if (tableExists) {
